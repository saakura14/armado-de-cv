  ...
  return (
    <div className="mt-4 space-y-3 rounded-2xl bg-papel p-5">
      ...
      <div className="flex flex-col gap-2 pt-2 sm:flex-row">
        <a aria-disabled={!ready} href={ready ? whatsappUrl(message) : undefined} target="_blank" rel="noreferrer" className={`flex min-h-12 flex-1 items-center justify-center gap-2 rounded-full bg-whatsapp px-5 py-3 font-display text-sm font-bold text-white ${ready ? '' : 'pointer-events-none opacity-50'}`}><WhatsAppIcon className="h-4 w-4" />Enviar por WhatsApp</a>
